...
local SKIN_ITEMS = {
    ...
    -- ── Buraya yeni skinler ekle ─────────────────────────
    -- Örnek: AKM
    -- 1101001174, -- Bạo Chúa Bộ Lạc - AKM (Cấp 8)
    -- Örnek: Araç
    -- 1903161,    -- Lamborghini Diablo GTR
}

local INS_BASE = 2100000000  -- Instance ID başlangıcı (çakışmasın diye 2.1 milyar)

local function InjectSkins()
    pcall(function()
        -- Wardrobe entity al
        local ok1, dc = pcall(require, "client.slua.logic.wardrobe.logic_wardrobe_data_center")
        if not ok1 or not dc then return end
        local ok2, entity = pcall(dc.GetWardrobeData)
        if not ok2 or not entity or not entity.AddData then return end

        local injected = 0
        for i, resID in ipairs(SKIN_ITEMS) do
            local insID = INS_BASE + resID  -- benzersiz instance ID
            pcall(function()
        ...
        if injected > 0 then
            ...
            -- Armory'ye de ekle (silah skinleri envanterde görünsün)
            pcall(function()
...
-- Lobby yüklendikten sonra çalıştır (2sn + 5sn)
pcall(function()
    local ok, t = pcall(require, "common.time_ticker")
    if ok and t and t.AddTimerOnce then
        t.AddTimerOnce(3.0, InjectSkins)
        t.AddTimerOnce(8.0, InjectSkins)
    end
end)
