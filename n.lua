    ...
    for _,enemy in pairs(allPawns) do
        if Valid(enemy) and enemy~=player then
            ...
            if alive then
                ...
                -- ── ESP TIPE 1 (FrameUI Kutucuk + HP% + Mesafe) ──────────
                if C.EspVip and HUD and distM <= 400 then
                    pcall(function()
                        ...
                        pcall(function()
                            ...
                            -- Box boyutunu mesafeye göre ölçekle
                            ...
                            if uiComp and slua.isValid(uiComp) then
                                ...
                                -- Mesafeye göre boyut: yakın=büyük, uzak=küçük
                                local boxScale = math.max(0.3, math.min(1.5, 80 / math.max(distM, 10)))
                                pcall(function()
                                    if uiComp.SetWorldSize then
                                        uiComp:SetWorldSize(boxScale * 100)
                                    end
                                    if uiComp.SetDrawSize then
                                        uiComp:SetDrawSize(FVector2D(boxScale * 120, boxScale * 60))
                                    end
                                end)
                            end
                        end)

                        -- HP YÜZDE (kutucuğun altında)
                        if not isKnock then
                            HUD:AddDebugText(string.format("HP:%d%%", hpPct), enemy, 0.06,
                                {X=0,Y=0,Z=100}, {X=0,Y=0,Z=100},
                                hpColor, true, false, true, nil, dynamicScale * 0.9, true)
                        else
                            HUD:AddDebugText("KNOCKED", enemy, 0.06,
                                {X=0,Y=0,Z=100}, {X=0,Y=0,Z=100},
                                C_RED, true, false, true, nil, dynamicScale * 0.9, true)
                        end

                        -- MESAFE
                        HUD:AddDebugText(string.format("[%dm]", math.floor(distM)), enemy, 0.06,
                            {X=0,Y=0,Z=150}, {X=0,Y=0,Z=150},
                            C_CYAN, true, false, true, nil, dynamicScale * 1.2, true)
                    end)
                else
                    pcall(function()
