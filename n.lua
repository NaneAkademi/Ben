...
-- M416 aldığında otomatik bu skin uygulanır (menüden seçmeye gerek yok)
-- Değiştirmek istersen sadece ID'yi değiştir
local WEAPON_SKIN_MAP = {
    [101004] = 1101004163,  -- M416 → Hoàng Gia Lộng Lẫy (Cấp 8)
    -- İstersen değiştir:
    -- [101004] = 1101004201,  -- M416 → Bạch Lân Nhả Ngọc
    -- [101004] = 1101004218,  -- M416 → Ma Ảnh
    -- [101004] = 1101004046,  -- M416 → Băng Giá
}
...
-- ── Tüm silahları tara ve skin uygula (equip_weapon_avatar) ──────
_G._CinedEquipAllWeapons = function(character)
    if not C.ModSkin then return false end
    if not character or not slua.isValid(character) then return false end
    local applied = false
    pcall(function()
        local WM = character:GetWeaponManager()
        if not WM or not slua.isValid(WM) then return end
        local wList = WM:GetAllInventoryWeaponList(false)
        if not wList or not slua.isValid(wList) then return end
        for i = 0, wList:Num() - 1 do
            pcall(function()
                local wep = wList:Get(i)
                if slua.isValid(wep) then
                    if ApplyWeaponSkin(wep) then applied = true end
                end
            end)
        end
    end)
...
-- ── 4) Armory Silah Skin Hook — silah skini seçince lokal uygula ──
pcall(function()
    local Arm = require("client.logic.armory.logic_armory")
    if not Arm or Arm._CinedWeaponHooked then return end
    Arm._CinedWeaponHooked = true

    -- install_weapon_skin_req → sunucuya sormadan lokal onayla
    local origInstall = Arm.install_weapon_skin_req
    if origInstall then
        Arm.install_weapon_skin_req = function(client_data, weapon_id, instanceID, ...)
            instanceID = tonumber(instanceID)
            if isInjectedIns(instanceID) then
                ensureNotExpired(instanceID)
                -- Sahte başarılı yanıt
                pcall(function()
                    if Arm.install_weapon_skin_rsp then
                        Arm.install_weapon_skin_rsp(client_data, 0, weapon_id, instanceID)
                    end
                end)
                return
            end
            return origInstall(client_data, weapon_id, instanceID, ...)
        end
    end

    -- send_install_weapon_skin_req (alternatif isim)
    local origSend = Arm.send_install_weapon_skin_req
    if origSend then
        Arm.send_install_weapon_skin_req = function(weapon_id, instanceID, ...)
            instanceID = tonumber(instanceID)
            if isInjectedIns(instanceID) then
                ensureNotExpired(instanceID)
                pcall(function()
                    if Arm.install_weapon_skin_rsp then
                        Arm.install_weapon_skin_rsp(nil, 0, weapon_id, instanceID)
                    end
                end)
                return
            end
            return origSend(weapon_id, instanceID, ...)
        end
    end
end)

-- ── 5) Wardrobe Gun Hook — silah skin giydirme ──────────────────
pcall(function()
    local wgl = require("client.slua.logic.wardrobe.logic_wardrobe_gun")
    if not wgl or wgl._CinedGunHooked then return end
    wgl._CinedGunHooked = true

    -- send_put_on_weapon_wear_req → lokal yanıt
    local origWear = wgl.send_put_on_weapon_wear_req
    if origWear then
        wgl.send_put_on_weapon_wear_req = function(self, weapon_id, skin_id, ...)
            skin_id = tonumber(skin_id)
            if isInjectedIns(skin_id) then
                ensureNotExpired(skin_id)
                pcall(function()
                    if wgl.on_put_on_weapon_wear_rsp then
                        wgl:on_put_on_weapon_wear_rsp(nil, 0, weapon_id, skin_id, nil)
                    end
                end)
                pcall(function()
                    if EventSystem and _G.EVENTTYPE_WARDROBE and _G.EVENTID_WARDROBE_UPDATE_ITEM_LIST then
            ...
            return origWear(self, weapon_id, skin_id, ...)
        end
    end

    -- OnClickWeaponSkin / OnSelectWeaponSkin → expire temizle
    for _, fn in ipairs({"OnClickWeaponSkin", "OnSelectWeaponSkin", "OnClickItem"}) do
        if wgl[fn] and not wgl["_CinedW_" .. fn] then
            wgl["_CinedW_" .. fn] = true
            local orig = wgl[fn]
            wgl[fn] = function(self, ...)
                pcall(function()
                    -- Tüm inject skinlerin expire'ını temizle
                    for ins, _ in pairs(_skinR.insToRes) do ensureNotExpired(ins) end
                end)
                return orig(self, ...)
            end
...
-- ── 6) DepotInit Hook — wardrobe açıldığında hemen inject ────────
pcall(function()
