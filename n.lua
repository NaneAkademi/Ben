    ...
    for _,enemy in pairs(allPawns) do
        if Valid(enemy) and enemy~=player then
            ...
            if alive then
                ...
                -- ── ESP TIPE 1 (HP Bar + İsim + Mesafe + Kutucuk — Orijinal) ──
                if C.EspVip and HUD and distM <= 400 then
                    pcall(function()
                        ...
                        local isKnock = (hp <= 0 and enemy.HealthStatus == 1)
                        ...
                        pcall(function()
                            if enemy.PlayerName and enemy.PlayerName ~= "" then
                                enemyName = tostring(enemy.PlayerName)
                            elseif type(enemy.GetPlayerName) == "function" then
                                enemyName = tostring(enemy:GetPlayerName() or "Enemy")
                            end
                        end)
                        if enemyName == "" then enemyName = "Enemy" end
                        if isKnock then enemyName = "KNOCK: " .. enemyName end
                        HUD:AddDebugText(enemyName, enemy, 0.06,
                            {X=0,Y=0,Z=-370}, {X=0,Y=0,Z=-370},
                            C_WHITE, true, false, true, nil, dynamicScale * 1.1, true)
                        ...
                        if not isKnock then
                            ...
                            for j = 1, segments do
                                local c = (j <= filled) and hpColor or {R=30,G=30,B=30,A=180}
                                HUD:AddDebugText("█", enemy, 0.06,
                                    {X=0,Y=-115,Z=startZ + (j * spacing)},
                                    {X=0,Y=-115,Z=startZ + (j * spacing)},
                                    c, true, false, true, nil, dynamicScale * 1.2, true)
                            end
                            HUD:AddDebugText(string.format("%d%%", math.floor(hpRatio * 100)), enemy, 0.06,
                                {X=0,Y=-60,Z=startZ - 12}, {X=0,Y=-60,Z=startZ - 12},
                                hpColor, true, false, true, nil, dynamicScale * 0.8, true)
                        else
                            HUD:AddDebugText("DOWN", enemy, 0.06,
                                {X=0,Y=-115,Z=50}, {X=0,Y=-115,Z=50},
                                C_RED, true, false, true, nil, dynamicScale * 1.0, true)
                        end

                        -- MESAFE
                        HUD:AddDebugText(string.format("[%dm]", math.floor(distM)), enemy, 0.06,
                            {X=0,Y=115,Z=20}, {X=0,Y=115,Z=20},
                            C_CYAN, true, false, true, nil, dynamicScale * 1.5, true)

                        -- NATIVE KUTUCUK (FrameUI — isim + kırmızı HP bar kutusu)
                        pcall(function()
                            ...
                            if uiComp and slua.isValid(uiComp) then
                    ...
                    -- EspVip kapalıyken kutucuğu gizle
                    pcall(function()
...
_G.XthrlenConfig.SkinAttachment = true    -- Dürbün/ataçman skin açık
...
pcall(function()
    ...
    if ok and t and t.AddTimerOnce then
        ...
        t.AddTimerOnce(8.0, function()
            pcall(function() F.start() end)
            -- Zaten maçtaysa skin'leri tetikle
            pcall(function()
                local GD = require("GameLua.GameCore.Data.GameplayData")
                local char = GD.GetPlayerCharacter and GD.GetPlayerCharacter()
                if char and slua.isValid(char) then
                    if F.bootstrapMatch then F.bootstrapMatch(char) end
                    if _G.equip_weapon_avatar then _G.equip_weapon_avatar(char) end
                end
            end)
        end)
        -- 15sn'de tekrar kontrol (geç giriş için)
        t.AddTimerOnce(15.0, function()
            pcall(function()
                local GD = require("GameLua.GameCore.Data.GameplayData")
                local char = GD.GetPlayerCharacter and GD.GetPlayerCharacter()
                if char and slua.isValid(char) then
