...
(async () => {
  ...
  const r = await p.evaluate(() => {
    ...
    g.spawnPickup(g.me.x + 30, g.me.z, 'rapid');
    const pk = [...g.pickups.values()].find(q => q.type === 'rapid');
    g.me.protT = 0;
  ...
  console.log('varil/bonus', JSON.stringify(await p.evaluate(() => {
    const g = window.app.game;
    return { barrelsAlive: g.world.barrels.filter(b => b.alive).length, rapidT: +g.me.rapidT.toFixed(1), pickups: g.pickups.size };
  })), JSON.stringify(r));
